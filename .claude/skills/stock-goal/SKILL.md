...
## CRITICAL: Single-Turn Continuous Execution
...
6. **단계 사이에 텍스트를 출력하지 않는다.**
   ...
   사용자에게 보이는 텍스트 출력은 최종 Completion Report 한 번뿐이다.
...
3. **멈추지 않고 즉시 Stage 4로 진행한다.**

### Stage 4: Image

1. plan, research, draft가 모두 존재하는지 확인한다.
2. `/stock-image` 스킬 계약에 따라 Codex CLI를 열어 `imagegen` skill로 hero 이미지 3장을 생성한다.
   - 프롬프트 3개 → PNG 3장 → 스코어링 → 1장 선택
   - `output/assets/<slug>-selected-image.json` 생성
3. Codex CLI 또는 이미지 생성 도구가 없으면 프롬프트와 blocked manifest만 남기고 `blocked`로 보고한다.
4. **멈추지 않고 즉시 Stage 5로 진행한다.**
...
4. **이제 Completion Report를 출력하고 턴을 종료한다.**
